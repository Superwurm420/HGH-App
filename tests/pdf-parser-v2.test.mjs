...
const uppercaseSubjectOnly = parsePdfTimetableV2(load('pdf-raw-grid-uppercase-subject-only.json'));
const uppercaseEntry = uppercaseSubjectOnly.model?.classes?.HT11?.mo?.[0];
if (!uppercaseEntry || uppercaseEntry.subject !== 'CAD' || uppercaseEntry.teacher !== '' || uppercaseEntry.room !== '') {
  throw new Error(`Expected uppercase-only subject to stay as subject (without fake teacher/room), got ${JSON.stringify(uppercaseEntry)}`);
}


const variableTeacherToken = parsePdfTimetableV2(load('pdf-raw-grid-variable-teacher-token.json'));
const variableTeacherEntry = variableTeacherToken.model?.classes?.HT11?.mo?.[0];
if (!variableTeacherEntry || variableTeacherEntry.subject !== 'Elektrotechnik' || variableTeacherEntry.teacher !== 'Müller-L' || variableTeacherEntry.room !== 'C12') {
  throw new Error(`Expected variable teacher token to be parsed without hardcoded pattern, got ${JSON.stringify(variableTeacherEntry)}`);
}
