 ...
 *
 * Bekannte Einschränkung: Es wird nur Seite 1 des PDFs ausgewertet.
 */

import { ParsedSchedule, LessonEntry, Weekday, WEEKDAYS } from './types';
...
export async function parseTimetablePdf(
  ...
  /** Text der Zeitspalte, ohne ein vorangestelltes Wochentagskürzel. */
  function leftColumnText(row: TextRow): string {
    ...
    for (const wd of WEEKDAYS) {
      if (text.startsWith(wd + ' ')) return text.slice(wd.length).trim();
    }
    return text;
  }
  ...
  const periodRowYs = rows
    .map((row) => {
      const leftText = leftColumnText(row);
